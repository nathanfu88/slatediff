 ...
 */
var makeMatrix = function makeMatrix(n, m, x) {
  var matrix = [];
  for (var i = 0; i < n; i++) {
 ...
 */
var diff = function diff(xs, ys) {
  var matrix = computeLcsMatrix(xs, ys);

  return printDiff(matrix, xs, ys, xs.size || 0, ys.size || 0);
};

var printDiff = function printDiff(matrix, xs, ys, xSize, ySize) {
  var diffArray = [];
  var i = xSize - 1;
  var j = ySize - 1;
  while (i >= 0 || j >= 0) {
 ...
 */
var backtrackLcs = function backtrackLcs(xs, ys, matrix) {
  var lcs = [];
